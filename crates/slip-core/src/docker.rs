...
impl DockerClient {
    ...
    /// port that Docker assigned on `127.0.0.1`. When `container_port` is `0`,
    /// no port binding is created and the returned host_port is `0` (used for
    /// worker apps that don't receive inbound HTTP traffic).
    ...
    pub async fn create_and_start(
        ...
        let port_bindings: Option<HashMap<String, Option<Vec<PortBinding>>>> = if container_port > 0
        {
