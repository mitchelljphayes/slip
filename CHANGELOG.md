...
- **Stop false-reporting the Tailscale Caddy certificate manager missing when the admin API `/modules/` endpoint returns 404 (SLIP-124).** The `/modules/` route is undocumented and always 404 on standard Caddy 2.x, so `has_cert_manager` propagated `Err` and `slip doctor`'s `.unwrap_or(false)` collapsed it into a false `Fail`. `slip doctor` now runs `caddy list-modules` via the injected `CommandRunner` (mirroring the DNS-plugin check) with exact module-ID matching (`tls.get_certificate.tailscale`); substrings no longer pass. Inconclusive checks (binary missing/nonzero) now `Warn` instead of `Fail`. The reconcile preflight keeps an API-first compatibility path with a binary fallback, and fail-closes on `Err` (unknown) with a prescriptive remedy rather than silently claiming absence. No public JSON schema or tri-state enum change.
